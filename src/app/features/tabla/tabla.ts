...
@Component({
  ...
  template: `
    @if (!ctx.activePollaId()) {
      ...
      <header class="phead">
        ...
        <a class="fame" routerLink="/stats" aria-label="Salón de la fama"><i class="ti ti-award"></i></a>
      </header>
      ...
      <section class="list">
        @for (s of rows(); track s.user_id; let i = $index) {
          <div class="row" [class.me]="isMe(s)">
            ...
            <app-avatar class="av" [url]="s.avatar_url" [name]="name(s)" />
            <div class="info">
              <div class="nm">{{ name(s) }} @if (isMe(s)) { <span class="you">Vos</span> }</div>
              <div class="rsub">{{ s.played }} jugados · {{ s.exacts }} exactos</div>
            </div>
            @if (s.movement > 0) {
              <span class="mv up" title="Subió {{ s.movement }}"><i class="ti ti-caret-up-filled"></i>{{ s.movement }}</span>
            } @else if (s.movement < 0) {
              <span class="mv down" title="Bajó {{ -s.movement }}"><i class="ti ti-caret-down-filled"></i>{{ -s.movement }}</span>
            } @else {
              <span class="mv flat"><i class="ti ti-minus"></i></span>
            }
  ...
  styles: `
    ...
    .phead { padding: 14px 14px 4px; display: flex; align-items: center; justify-content: space-between; gap: 10px; }
    ...
    .fame { width: 40px; height: 40px; border-radius: 50%; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: var(--color-background-warning); color: var(--color-text-warning); text-decoration: none; }
    .fame i { font-size: 22px; }
    ...
    .mv { display: inline-flex; align-items: center; gap: 1px; font-size: 11px; font-weight: 700; font-family: var(--font-display); }
    .mv i { font-size: 13px; }
    .mv.up { color: var(--color-text-success); }
    .mv.down { color: var(--color-text-danger); }
    .mv.flat { color: var(--color-text-tertiary); }
    .mv.flat i { font-size: 11px; }
    .pts { text-align: right; display: flex; flex-direction: column; line-height: 1; min-width: 38px; }
